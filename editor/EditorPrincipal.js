...
import { EditorAudio } from './EditorAudio.js';
...
class EditorPrincipal {
    constructor(canvas) {
        ...
        this.editorAudio = new EditorAudio(this);
    ...
    configurarToolbar() {
        ...
        document.getElementById('btn-audio-manager')?.addEventListener('click', () => {
            if (this.editorAudio) {
                this.editorAudio.abrir();
            } else {
                this.log('Editor de Áudio não inicializado.', 'error');
            }
        });
        ...
        document.getElementById('btn-toggle-mute')?.addEventListener('click', (e) => {
            if (window.AudioManager) {
                ...
                e.currentTarget.style.color = muted ? '#ff6b6b' : '#fff';
                this.log(`Áudio ${muted ? 'Mutado' : 'Desmutado'}`, 'info');
            }
        });
    ...
    aplicarProjeto(dados) {
        ...
        if (dados.pastas) {
            ...
            // FIX: Forçar todas as pastas a iniciarem fechadas ao carregar projeto
            this.pastas.forEach(p => p.aberta = false);
        }
    ...
    criarEntidade(tipo) {
        ...
        if (!pastaCena && (tipo === 'tilemap' || this.entidades.length === 0)) {
            ...
            pastaCena = {
                id: 'folder-' + Date.now(),
                nome: 'Cena 01',
                aberta: false // FIX: Default Closed
            };
    ...
    alternarModo() {
        ...
        if (this.modoEdicao) {
            ...
            // FIX: Resetar timer do loop para evitar "salto" de deltaTime causado pelo tempo de inicialização (IO/Audio)
            // Isso previne que a física (gravidade) faça o player atravessar o chão no primeiro frame.
            if (this.engine.loopJogo) {
                this.engine.loopJogo.tempoAnterior = performance.now();
            }
        ...
        if (!tileComp || !tilemapEnt) {
            ...
            if (this.entidades.filter(e => e.obterComponente('TilemapComponent')).length === 0) {
                ...
                if (pastaCena) novaEntidade.pastaId = pastaCena.id;
                    ...
                    const novaPasta = { id: 'folder-' + Date.now(), nome: 'Cena 01', aberta: false };
    ...
    atualizarPainelPropriedades() {
                ...
                <div style="margin-bottom:5px;">
                 ...
                 <div>
                    ...
                    <div style="display:flex; gap:5px; align-items: center;">
                         <input type="number" id="prop-kz-x" value="${killRef.resetPosition.x}" style="flex:1; background:#111; color:white; border:1px solid #444; padding:5px; border-radius:4px;width: 80px;">
                         <input type="number" id="prop-kz-y" value="${killRef.resetPosition.y}" style="flex:1; background:#111; color:white; border:1px solid #444; padding:5px; border-radius:4px;width: 80px;">
                         <button id="btn-kz-set-view" title="Definir para Centro da Câmera" style="background:#444; border:none; color:white; cursor:pointer; padding:5px; border-radius:4px;">📍</button>
                    </div>
                </div>
            </div>`;
            ...
            <div style="background:#222; padding:5px; border-radius:4px; margin-bottom:5px;">
                ...
                <div style="margin-top:5px; padding-top:5px; border-top:1px dashed #444;">
                    ...
                    <label style="font-size:10px; color:#aaa;">${i18n.t('item.pickupSound')}</label>
                     <div class="audio-dropzone" data-target="item-audio-${itemComp.itemId}" style="display:flex; gap:5px; border:1px dashed #f1c40f; padding:4px; border-radius:4px; margin-bottom:5px; background: rgba(241, 196, 15, 0.1);">
                        <select id="item-audio-${itemComp.itemId}" class="plugin-prop" data-plugin="ItemComponent" data-prop="pickupSound" style="flex:1; background:#111; color:#f1c40f; border:none; padding:5px;">
                            <option value="">${i18n.t('item.defaultNone')}</option>
                            ${(() => {
                    let opts = '';
                    if (this.assetManager && this.assetManager.assets.sons) {
                        this.assetManager.assets.sons.forEach(asset => {
                            const sel = (asset.id === itemComp.pickupSound) ? 'selected' : '';
                            opts += `<option value="${asset.id}" ${sel}>🎵 ${asset.nome}</option>`;
                        });
                    }
                    return opts;
                    ...
                    <label style="font-size:10px; color:#aaa;">${i18n.t('item.pickupPitch')}</label>
                    <input type="number" step="0.1" value="${itemComp.pickupPitch !== undefined ? itemComp.pickupPitch : 1.0}" class="plugin-prop" data-plugin="ItemComponent" data-prop="pickupPitch" style="width:100%; background:#111; color:white; border:1px solid #444; padding:5px; margin-bottom:5px;">
        ...
        // 2.16 SoundComponent
        const soundComp = ent.obterComponente('SoundComponent');
        if (soundComp) {
            let soundHtml = `
            <div style="background:#222; padding:5px; border-radius:4px; margin-bottom:5px;">
                <div style="margin-bottom:5px;">
                     <label style="font-size:10px; color:#aaa;">${i18n.t('prop.assetId')}</label>
                     <div class="audio-dropzone" data-target="audio-selector" style="display:flex; gap:5px; border:1px dashed #444; padding:2px; border-radius:4px;">
                        <select id="audio-selector" class="plugin-prop" data-plugin="SoundComponent" data-prop="assetId" style="flex:1; background:#111; color:#f1c40f; border:none; padding:5px;">
                            <option value="">(Nenhum)</option>
                            ${(() => {
                    let opts = '';
                    if (this.assetManager && this.assetManager.assets.sons) {
                        this.assetManager.assets.sons.forEach(asset => {
                            const sel = (asset.id === soundComp.assetId) ? 'selected' : '';
                            opts += `<option value="${asset.id}" ${sel}>🎵 ${asset.nome}</option>`;
                        });
                    }
                    return opts;
                ...
                <div style="margin-bottom:5px;">
                     <label style="font-size:10px; color:#aaa;">${i18n.t('prop.volume')}: <span style="color:#f1c40f;">${Math.round(soundComp.volume * 100)}%</span></label>
                     <input type="range" min="0" max="1" step="0.1" value="${soundComp.volume}" class="plugin-prop" data-plugin="SoundComponent" data-prop="volume" style="width:100%;">
                </div>

                <div style="display:flex; flex-wrap:wrap; gap:10px; margin-bottom:10px;">
                     <label style="font-size:11px; color:#ccc; display:flex; align-items:center; cursor:pointer;">
                        <input type="checkbox" class="plugin-prop-check" data-plugin="SoundComponent" data-prop="loop" ${(soundComp.loop) ? 'checked' : ''} style="margin-right:5px;"> ${i18n.t('prop.loop')}
                    </label>
                    <label style="font-size:11px; color:#ccc; display:flex; align-items:center; cursor:pointer;">
                        <input type="checkbox" class="plugin-prop-check" data-plugin="SoundComponent" data-prop="autoplay" ${(soundComp.autoplay) ? 'checked' : ''} style="margin-right:5px;"> ${i18n.t('prop.autoplay')}
                    </label>
                    <label style="font-size:11px; color:#ccc; display:flex; align-items:center; cursor:pointer;">
                        <input type="checkbox" class="plugin-prop-check" data-plugin="SoundComponent" data-prop="isMusic" ${(soundComp.isMusic) ? 'checked' : ''} style="margin-right:5px;"> ${i18n.t('prop.isMusic')}
                    </label>
                </div>

                <div style="display:flex; gap:5px;">
                    <button class="btn-preview-sound" data-action="play" style="flex:1; background:#27ae60; color:white; border:none; padding:5px; border-radius:3px; cursor:pointer;">▶ Play</button>
                    <button class="btn-preview-sound" data-action="stop" style="flex:1; background:#c0392b; color:white; border:none; padding:5px; border-radius:3px; cursor:pointer;">⏹ Stop</button>
                </div>
            </div>`;
            componentsHtml += createComponentHtml('SoundComponent', i18n.t('comp.soundComponent'), '🎵', '#e67e22', soundHtml, true);
        }
    ...
    renderizarCamposScript(scriptComp, scriptId) {
        ...
        if (scriptComp.instance) {
            // FAILSAFE CRÍTICO: Para StatsRPG, garante que propriedades de áudio existam
            // Isso resolve o problema de saves antigos que não têm essas propriedades
            if (scriptComp.instance.constructor.name === 'StatsRPG') {
                if (scriptComp.instance.somLevelUp === undefined) {
                    scriptComp.instance.somLevelUp = '';
                }
                if (scriptComp.instance.pitchLevelUp === undefined) {
                    scriptComp.instance.pitchLevelUp = 1.0;
                }
            }

            const props = Object.keys(scriptComp.instance).filter(k => k !== 'entidade' && !k.startsWith('_'));

            // DEBUG: Log para StatsRPG
            if (scriptComp.instance.constructor.name === 'StatsRPG') {
                console.log('[DEBUG] StatsRPG properties:', props);
                console.log('[DEBUG] somLevelUp exists?', 'somLevelUp' in scriptComp.instance);
                console.log('[DEBUG] somLevelUp value:', scriptComp.instance.somLevelUp);
            }
            ...
            props.forEach(prop => {
                ...
                // Detecta áudio por nome (som*) OU por metadata (tipo: 'audio')
                // Nota: Aceita strings vazias também (valor === '')
                let isAudioProp = prop.startsWith('som') && (typeof valor === 'string' || valor === '');

                // Se não detectou por nome, verifica metadata
                if (!isAudioProp && scriptComp.instance.constructor.metadata) {
                    const metadata = scriptComp.instance.constructor.metadata;
                    if (metadata.propriedades) {
                        const propMeta = metadata.propriedades.find(p => p.nome === prop);
                        if (propMeta && propMeta.tipo === 'audio') {
                            isAudioProp = true;
                        }
                    }
                }
                ...
                if (isAnimProp) {
                    ...
                    html += `<div class="audio-dropzone-script" data-script-id="${scriptId}" data-prop="${prop}" style="border:1px dashed #f1c40f; padding:4px; font-size:11px; color:#ffeaa7; cursor:default; background: rgba(241, 196, 15, 0.1);">🎵 ${valor || '(Solte Audio Aqui)'}</div>`;
    ...
    configurarListenersPropriedades(ent) {
        ...
        document.getElementById('btn-kz-set-view')?.addEventListener('click', () => {
            const k = getKZ();
            if (k && this.engine.renderizador) {
                const cam = this.engine.renderizador.camera;
                const cvs = this.engine.renderizador.canvas;
                // Calcula centro da tela em coordenadas do mundo
                k.resetPosition.x = Math.floor(cam.x + (cvs.width / 2));
                k.resetPosition.y = Math.floor(cam.y + (cvs.height / 2));
                this.atualizarPainelPropriedades();
            }
        });
        ...
        // --- Drag & Drop: Script Audio Binding (NOVO) ---
        document.querySelectorAll('.audio-dropzone-script').forEach(zone => {
            zone.addEventListener('dragover', e => {
                e.preventDefault();
                e.dataTransfer.dropEffect = 'copy';
                zone.style.borderColor = '#ffeaa7';
                zone.style.background = '#3a3a2e';
            });
            zone.addEventListener('dragleave', e => {
                zone.style.borderColor = '#f1c40f';
                zone.style.background = 'rgba(241, 196, 15, 0.1)';
            });
            zone.addEventListener('drop', e => {
                ...
                try {
                    const data = JSON.parse(dataStr);
                    // Aceita apenas asset_audio
                    if (data.tipo === 'asset_audio') {
                        const scriptId = zone.dataset.scriptId;
                        const propName = zone.dataset.prop;
                        const scriptComp = ent.obterComponente(scriptId);

                        if (scriptComp && scriptComp.instance) {
                            scriptComp.instance[propName] = data.id; // Salva o ID do asset
                            this.atualizarPainelPropriedades();
                            this.log(`Audio vinculado: ${data.id}`, 'success');
                        }
                    } else {
                        this.log('Apenas arquivos de áudio podem ser soltos aqui!', 'warning');
                    }
        ...
        // Sound Component Listeners
        document.querySelectorAll('.btn-preview-sound').forEach(btn => {
            btn.addEventListener('click', () => {
                const soundComp = ent.obterComponente('SoundComponent');
                if (soundComp) {
                    const action = btn.dataset.action;
                    if (action === 'play') soundComp.play();
                    else if (action === 'stop') soundComp.stop();
                }
            });
        });

        document.querySelectorAll('.btn-select-audio').forEach(btn => {
            btn.addEventListener('click', () => {
                const soundComp = ent.obterComponente('SoundComponent');
                if (soundComp) {
                    const newId = prompt(i18n.t('prop.assetId'), soundComp.assetId);
                    if (newId !== null) {
                        soundComp.assetId = newId;
                        this.atualizarPainelPropriedades();
                    }
                }
            });
        ...
        // Audio Dropzone Listener
        document.querySelectorAll('.audio-dropzone').forEach(zone => {
            zone.addEventListener('dragover', e => {
                e.preventDefault();
                e.dataTransfer.dropEffect = 'copy';
                zone.style.borderColor = '#4ecdc4';
                zone.style.background = '#222';
            });
            zone.addEventListener('dragleave', e => {
            ...
            zone.addEventListener('drop', e => {
                ...
                try {
                    ...
                    const data = JSON.parse(dataStr);
                    // Aceita tanto 'asset_audio' quanto check generico de ID
                    if (data.tipo === 'asset_audio') {
                        const selectorId = zone.dataset.target;
    ...
    abrirUIBuilder(uiComponent) {
        ...
        // Asset Picker Button
        modal.querySelector('#btn-pick-asset').onclick = () => {
            const assetInput = modal.querySelector('#el-assetId');

            // Create asset picker modal
            const picker = document.createElement('div');
            picker.style.cssText = `
                position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%);
                width: 400px; max-height: 500px; background: #222; border: 2px solid #4ecdc4;
                box-shadow: 0 0 30px rgba(78, 205, 196, 0.3); z-index: 3500; border-radius: 8px;
                display: flex; flex-direction: column; overflow: hidden;
            `;

            picker.innerHTML = `
                <div style="padding:10px; background:#333; border-bottom:1px solid #4ecdc4; display:flex; justify-content:space-between; align-items:center;">
                    <h4 style="margin:0; color:#4ecdc4;">🖼️ Select Image Asset</h4>
                    <button id="btn-close-picker" style="background:none; border:none; color:#4ecdc4; cursor:pointer; font-size:16px;">✖</button>
                </div>
                <div id="asset-list" style="flex:1; overflow-y:auto; padding:10px;"></div>
            `;

            document.body.appendChild(picker);

            const assetList = picker.querySelector('#asset-list');

            // Populate with sprite assets
            if (this.assetManager && this.assetManager.assets.sprites) {
                this.assetManager.assets.sprites.forEach(asset => {
                    const item = document.createElement('div');
                    item.style.cssText = `
                        padding: 8px; margin-bottom: 5px; background: #333; border-radius: 4px;
                        cursor: pointer; border: 1px solid #444; transition: all 0.2s;
                    `;
                    item.innerHTML = `<span style="color:#4ecdc4;">🖼️</span> ${asset.nome}`;

                    item.onmouseenter = () => item.style.background = '#4ecdc4';
                ...
                assetList.innerHTML = '<div style="color:#666; text-align:center; padding:20px;">No sprite assets available</div>';
            }

            picker.querySelector('#btn-close-picker').onclick = () => document.body.removeChild(picker);
        };
    ...
    abrirMenuAdiocionarComponente() {
        ...
        const categorias = {
            ...
            [t('category.gameplay')]: [
                ...
                { id: 'SoundComponent', nome: t('comp.soundComponent'), icon: '🎵', unico: true }
            ],
            [t('category.scripts')]: [
        ...
        modal.querySelectorAll('.btn-select-comp').forEach(btn => {
            btn.onclick = () => {
                ...
                } else if (tipo === 'SoundComponent') {
                    ent.adicionarComponente('SoundComponent', new SoundComponent(ent));
                }
    ...
    async salvarProjeto() {
        try {
            this.log('Compactando projeto...', 'info');
            ...
            a.href = url;
            a.download = 'projeto.lumina'; // Extensão binária
            a.click();
            URL.revokeObjectURL(url);

            this.log('Projeto salvo e compactado com sucesso! (.lumina)', 'success');
        } catch (err) {
            console.error(err);
            this.log('Erro ao salvar projeto: ' + err.message, 'error');
        }
    }
    ...
    restaurarEstadoInicial() {
        ...
        if (snapshot.pastas) {
            ...
            // FIX: Ensure folders are closed even when restoring state (Play -> Edit)
            this.pastas.forEach(p => p.aberta = false);
        }
            ...
            if (dado.componentes) {
                ...
                for (const dadosComp of listaComponentes) {
                    ...
                    try {
                        ...
                        } else if (dadosComp.tipo === 'SoundComponent') {
                            componente = new SoundComponent();
                        }
                        ...
                        if (componente) {
                            ...
                            if (componente instanceof ScriptComponent && componente.id) {
                                ...
                                if (componente.scriptName === 'MovimentacaoPlataformaScript' ||
                                    componente.scriptName === 'MovimentacaoAvancada' ||
                                    ...
                                    if (needsUpdate && typeof GeradorScript !== 'undefined') {
                                        ...
                                        try {
                                            ...
                                            if (componente.scriptName === 'StatsRPG') {
                                                newSource = gerador.gerarScriptStatsRPG();
                                            } else if (componente.scriptName === 'SimuladorMorte') {
                                                newSource = gerador.gerarScriptSimuladorMorte();
                                            } else if (componente.scriptName === 'DeathScreenScript') {
                                                // Verifica qual o tipo de morte
                                                ...
                                                newSource = gerador.gerarMovimentacaoPlataforma(infoSimulado);
                                            }
                                        ...
                                        console.log('[Auto-Patch] Script já está atualizado');
                                    }
                                }
    ...
    carregarProjeto(evento) {
        ...
        leitor.onload = async (e) => {
            try {
                ...
                // Tentar descompactar (GZIP)
                try {
                    // Clone o buffer porque pode ser usado apenas uma vez
                    ...
                    this.log('Projeto descompactado com sucesso!', 'info');
                } catch (gzipErr) {
                    // Fallback: Arquivo não comprimido (Legacy JSON)
                    // Como lemos como ArrayBuffer, precisamos decodificar o texto
                    const dec = new TextDecoder("utf-8");
                    jsonString = dec.decode(e.target.result);
                }

                const dados = JSON.parse(jsonString);

                this.aplicarProjeto(dados);
        ...
        // Ler como ArrayBuffer para suportar binários (GZIP)
